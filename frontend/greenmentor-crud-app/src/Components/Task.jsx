...
const TaskList = () => {
  ...
  const { isLoading, isError, Task } = useSelector(
    (state) => state.postReducer
  );
  const { token } = useSelector((state) => state.userReducer);
  console.log({ Task });
  const handleDelete = (id) => {
    dispatch(DeletelProduct(id, token))
      .then((res) => {
        getAllTaskList();
      })
      .catch((err) => {});
  ...
  const handleUpdate = (id) => {
    ...
    dispatch(updateTask(id, updatedTask, token))
      .then((res) => {
        setisUpdate(1);
        setUpdateUser({});
  ...
  return isLoading ? (
    ...
    <div className="max-w-2lg min-h-[77vh] mx-auto bg-gray-100 sm:p-[20px]">
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50 text-center font-bold">
              <tr>
                <th
                  scope="col"
                  className="px-6 py-4 text-xs font-medium tracking-wider text-center uppercase"
                >
                  Title
                </th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {Task.map((task, index) => (
                ...
                >
                  {isUpdate == task._id ? (
                    <td className="px-6 py-4 whitespace-nowrap text-wrap text-justify">
                      <input
