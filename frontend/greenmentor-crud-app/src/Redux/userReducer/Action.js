...
export const LoginRequest = payload => async dispatch => {
   try {
      ...
      return await axios.post(`${userBasheUrl}/login`, payload).then(res => {
...
export const RgisterRequest = payload => async dispatch => {
   try {
      dispatch({ type: LOGIN_REQUEST })
      return await axios.post(`${userBasheUrl}/register`, payload).then(res => {

         dispatch({ type: REGISTER_REQUEST_SUCCESS, payload: res.data })
         if(res?.data?.token){

            localStorage.setItem("greenmentortoken", JSON.stringify(res.data.token))
         }
         return res
      }).catch(error => {
...
export const LogoutRequest = (token) => async dispatch => {
   try {
      dispatch({ type: LOGIN_REQUEST })
      return await axios.get(`${userBasheUrl}/logout`, { headers: { Authorization: token } }).then(res => {
